...
export async function GET(
  ...
  const variantsRes =
    productIds.length > 0
      ? await supabase
          ...
          .select("id, product_id, name, sku, price, currency_code, pack_size, uom, is_default, is_active")
          .eq("tenant_id", tenantId)
  ...
  for (const row of attrValuesRes.data ?? []) {
    ...
    current.push({
      ...
      code: def.code,
      name: def.name,
      value: extractValue(row.value_jsonb),
    });
  ...
  return NextResponse.json({
    ...
    variants: variants.map((variant) => ({
      ...
      productName: productById.get(variant.product_id)?.name ?? "",
      name: variant.name,
      sku: variant.sku,
      price: variant.price,
